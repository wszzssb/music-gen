...
#### 再往下一层：**为什么**不吐 `<eos>`（2026-09-26 归因实验 · 同一首曲 / 同会话）

判据仍是解码步数（越接近 256 = 越"不知道停"），只改**喂什么**：

| 条件 | 每批解码步数 | 中位 | s/段 |
|---|---|---|---|
| 全混音（基线） | `[167, 82, 109]` | 109 | 0.152 |
| `h6_other` 单条（基线） | `[256, 162, 256]` | 256 | 0.602 |
| **`other` + `bass` 两条** | **`[169, 92, 115]`** | **115** | **0.151** |
| 六条分轨全回混 | `[182, 115, 212]` | 182 | 0.205 |
| `other` 低通 8k / 4k | 242 / 240 | — | 0.43 / 0.39 |
| `other` 高通 200Hz | 210 | — | 0.36 |

**主因 = 织体不完整，不是频谱**：给 `other` **只加一条 bass**，256 步 → **115 步**，
直接回到全混音水平（109）。模型要的是"**和声 / 低频上下文**"才认得出"曲子结束了"——
一件中高音乐器单独喂进去，它不知道该在哪停。

**次因 = 分离产物自身的分布外成分**，但很小：六条全回混仍 **182 步**（比两条还差，
因为 Demucs 的 piano/guitar/vocals 支路本身带噪）；而回混音频与原曲的频谱差
只有 **0.1~1.5 dB**（1/3 倍频程带）—— 所以**不是"频谱对不上"**，滤波也只值 15~45 步。
→ 这是个**"上下文"问题，不是"音质"问题**：靠滤波 / 归一化修不动（电平已在上一轮 2×2 排除）。
