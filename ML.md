...
### ⚠ 分轨输入会慢 3–11 倍：**模型不吐 `<eos>`**（2026-09-26 实测 · 扒 `princess_charm` 时发现）

**症状**：拿 `transcribe_ymt3.py` 跑 Demucs **分轨**，比同一首曲子的**全混音**慢好几倍
（实测 **0.552 / 0.600 s/段** vs **0.148**），而"分组推理 + `bsz auto`"**全都生效了** ——
输出只有一行 "0.600 s/段"，看着和"这首曲子难"没区别，**报错为零**。

**真因（读 `amt/src/model/t5mod_helper.py:96-131`）**：解码是**自回归循环**，
最多 `max_total_token_length`（本权重 **256**）步，**只有整个 batch × 13 通道全部吐出
`<eos>` 才提前 break**（`unfinished_sequences.max() == 0`）
→ **耗时 ∝ 实际解码步数**，而步数由**最慢的那一条**决定。
`inference_file` 返回的 `pred_token_arr[k].shape[-1]` **就是实际步数**（可直接量）：

| 输入（同一首 107s 曲 · bsz=24 · 同机同会话） | 每批实际解码步数 | s/段 |
|---|---|---|
| 全混音 | `[167, 90, 109]` | **0.148** |
| `h6_drums` 分轨 | `[39, 42, 42]` | 0.054 |
| **`h6_other` 分轨** | **`[256, 112, 256]` = 撞上限** | **0.600** |

**分轨是分布外输入 → 模型不知道该在哪结束 → 一路空转到上限。**

**排除的三个竞争假设**（都是现场实测，不是推断）：
① **不是显存**（峰值 4094 vs 4965 MB，两者都远低于 8GB）；
② **不是分组**（两边都是"分 2 组 · 每组 ≤29 段"）；
③ **不是电平** —— 2×2 交叉：混音**压 12dB** 一模一样（0.148→**0.142**）、
分轨**抬 12dB** 也只 0.534→**0.480**（仍 244 步）→ 是**内容**不是振幅。

**怎么办**：
- `transcribe_ymt3.py` 现在**自报解码步数**，撞上限会当场打印原因
  （守卫 `t_ymt3_reports_decode_steps` + 变异用例 0b2b）；
- **纯 `restore_oneshot` 路径可以少跑分轨**：分轨转录只喂 `probe_instruments` 的
  **第①层（YMT3 通道初筛）**，而真正定编制的是第③b/③c 层（分轨 **wav** 的能量/起音，
  那是 Demucs 的产物、与这 10 条 MIDI 无关）。实测那第①层给出"**标签冲突 0 段**"、
  **没改变任何决定**，而它是整条链**最贵的一步**（10 条分轨 3m42s）；
- 走**集成转录**（`imitate_ref.py` 那条路）**要**跑分轨 —— 那属必要成本。
