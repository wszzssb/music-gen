...
- **交付/产出的文件，一律给完整绝对路径**（用户 2026-09-20 定，**不限音乐任务**）：
  写成能直接粘进资源管理器 / 终端的形式（如 `D:\work\build\report.md`、`D:\test\dt_midi\x.mid`），
  **别只说文件名，也别用"在当前目录 / 在 xx 文件夹里 / 就在仓库下"代替**；
  多个产物逐行列全 —— 实测"找不到文件"会白耗一整轮。
- **临时/中间文件先建专用目录**（用户 2026-09-20 定）：临时脚本与中间产物一律放
  `D:\test\_tmp\<任务短名>\`（如 `D:\test\_tmp\midi-extract\`），**别散落在 `D:\test` 根下** ——
  实测根目录积了 **145 个**散落文件（90 个 `_*.py`/`_*.sh` 分不清谁留的、也不敢删）。
  **交付物不放这里面**（那是要留的）；任务收尾或用户说清理时**整个目录删掉**即可。
