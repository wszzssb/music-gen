...
4. **它判不了"哪版更好"，但能对比"原版 vs 当前版"的差异** —— 前者是**价值判断**（§6.1），
   后者是**同段同问对照**（`--compare`，实测可用，见 **§6.0**）。别再一律写成"不能比"。
...
### 6.0 **能对比"原版 vs 当前版"的差异**（2026-09-25 晚补；用户口径）

本节 6.1 起讲的是"**当 A/B 尺子**"为什么不行（判**好坏**）。**判差异 ≠ 判好坏** ——
`--compare 原版.wav 当前版.wav --segments 6 --load-4bit --ask "<同段同问>"` 实测可用：
同一段、同一个问题分别问两版，**它自己报出来的乐器/角色/进出方式的差别**就是线索。
本轮实例（`siren_end2` 扒带，6 段）：段 4（88–118s）它说原版"**小提琴演奏主旋律**"、
我这版"**钢琴演奏主旋律（+爵士鼓和小号）**"；段 5（118–147s）同样。
这两条与**用户原话**（"104–130 大部分弦乐"）和 **`probe_instruments` 的分轨能量证据**
**三方一致** —— 是这条路上第一次三方对上（此前 §8-1 那次与耳朵重合 0）。
**边界照旧**：① 它说的是**差异**，不是**优劣**；② 单次读数会漂（§6.1 的 0.2dB 微扰）；
③ "问题类别矩阵"仍然恒真，别用矩阵当判据，要读**它描述的内容**；
④ 它仍会在**已知真值**的窗口上说错（同一轮：69–96s 用户说人声吟唱，它说钢琴主奏）。

### 6.1 为什么不能当 A/B 尺子（**判好坏**）
...
| 能用它做的 | **不能**用它做的 |
|---|---|
| 大致定位"**哪一段听着可疑**"（大尺度） | 判"这版比那版好**吗**"（**好坏 = 价值判断**） |
| **对比"原版 vs 当前版"的差异**（`--compare` 同段同问，读它描述的内容；§6.0） | 把"差异"当成"优劣"（差在哪里 ≠ 哪版更对） |
