...
### 口径：「写 MIDI」这一层的参考画像**只用 `refs/midi2/`**（用户 2026-09-21 定）

| 阶段 / 路径 | 参考画像来自哪 | 例外 |
|---|---|---|
| **写 MIDI**（直接作曲：和声 / 节奏音型 / 配器 / 曲式 / 旋律语言 / 音色池） | **只能 `refs/midi2/`**（或网络权威数据，带来源 URL） | 无 |
| **混音对标**（响度 / 宽度 / 质心 / 频段 / 段间起伏） | `refs/*.json` 真实录音 → 聚合 `<主题>_mix.json` | 无 |
| **仿写**（照着某一首写） | 用户给的**那一首**参考曲画像（`profile_ref.py`） | 本就该用单首 |
| **提取 MIDI**（扒带 / 还原） | 同上：那一首参考曲的分轨 / 转录 | 本就该用单首 |

用户原话："以后全部参考画像都用 midi2 的文件，**除了仿写和提取 midi**"，
并补充"**我说是写 midi 时**" —— 即**写 MIDI 这一层的依据**必须能溯源到 `refs/midi2/`；
仿写与提取 MIDI 的依据本来就是用户给的那一首参考曲（音频），不在此列。
**混音对标仍用真实录音画像**（MIDI 渲染 ≠ 真实录音，别拿它当频谱目标）。
