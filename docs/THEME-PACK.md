...
> **这条路径管"直接作曲"**（"来一首 BGM"）：依据 = 主题模板包，**没有参考曲蓝本**。
> 另外两条是 —— **仿写**（照着某首参考曲做，`docs/IMITATE-PATH.md`）与
> **提取 MIDI / 还原扒带**（把参考曲转录成 MIDI，`docs/RESTORE-METHOD.md`）。
> **三条路径的依据不同，混用会静默走样**（口径与红线见 `docs/IMITATE-PATH.md` 文首那张表）。
...
## 4b. 好听的量化判据（2026-09-19 从还原链反推 —— **写歌时也按这个自检**）

用户对成品的听感反馈最后收敛成两句：**"挺好听"** 与 **"还是不像"**。
这两句对应**两组不同的判据** —— 前者能靠参数修，后者是音高内容问题（`RESTORE-METHOD.md` §0）：

| 判据 | 目标 | 实测依据（同一把尺子） |
|---|---|---|
| **旋律轨时值中位** | ≥ **0.25s** | 认可版 v5：Piano 0.266s · Bass 0.232s · Guitar 0.345s |
| **旋律轨碎音率**（时值 ≤0.25 拍的音占比） | **< 5%** | v5：2.7% / 2.7% / 3.9%。修前我的是 30.4% / 22.1% / **92.5%** → 用户判"太杂乱、不流畅"；拉到 0~13.6% 后变成"**挺好听**" |
| **打击轨碎音率** | **~100%（别动它）** | v5 的 Perc 也是 0.010s / 100% —— 鼓本来就该是碎音，**拉长鼓只会糊成一团** |
| **声部 ↔ 音色** | 主体声部不能塞错音色 | 原曲 `other`（合成器/弦乐）占能量 44%、钢琴只占 7.9% 的曲子，把合成器音并进 Piano = 音高对、音色错 = 听感"**不像**" |

**怎么用**：

```bash
# 量一首（或与参照对照）：逐轨时值中位 / 碎音率 / 低于下限占比
python scripts\note_dur_stats.py <我的.mid> [对照.mid] --floor 0.55
```

⚠ **三条硬纪律**（都是实测踩出来的）：
1. **时值下限是"延长"，不是"合并"** —— 所有音的**起点一个不动**（onset 集合不变），只改终点。
   所以它和 `RESTORE-METHOD.md` §2 那条"碎片合并（同音高 ≤40ms）→ F1 0.481→0.283"
   **不冲突**：那条反对的是**删掉 onset**。
2. **一律换算成秒再比** —— 本链的 MIDI 常是 120BPM 语义、参考曲是 150BPM，
   同一个"0.55 拍"在两边不是同一个时长。比"拍"会得出错的倍数（实测踩过）。
3. **"像不像"别拿时值/碎音去修** —— 那是音高内容问题。判据先跑识别体检
   （`transcribe_audit.py` 一致率 <70% 就是"先修识别"，不是调混音/音色/EQ）。
