...
5. **每份文档都有预算**：超了 FAIL；调上限要给理由（如"台账增长 → 拆归档"）。
   **先抬上限，后压缩**（用户 2026-09-17 定）：新内容**先落进去**、上限跟着调到当前值，
   **不让警告挡住正在做的事**；随后**必须**补压缩 —— 这是**欠账，不是可选项**。
   抬上限时**同时记下压缩目标值**（如"目标回到 1150"），压到位后再调回来。
...
**0. 动手前先抬上限**（用户 2026-09-17 定的"**先加大预算，最后再压缩**"）——
**任何**往有预算文档里加内容的动作，**第一件事是改 `token_audit.LIMITS`**，
**然后才落内容**，最后按 §1 第 5 条补压缩。

**抬多少 =「当前值 + 预估增量 + 余量」，宁可先抬宽**（反正压到位要调回来；
**抬窄了等于没抬**）。中文按 **≈0.9 tok/字** 估。
**自查**：动文档前先 `grep '<文档名>' scripts/token_audit.py` 看上限，顺手抬。

**三次翻车记录（同一件事的三个错法）**：
① **忘了抬**（`CONVENTION.md` +130 tok 顶过 1400）；② **超了才抬**（`SKILL.md` +137 字符顶过 4200）；
③ **抬了但抬窄**（CONVENTION 抬到 1600 只留 109 余量，而那轮实增 173 tok → 仍超 1664）。
三次都得回头改提交。
