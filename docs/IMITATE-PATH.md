...
| ③ | 量**每 8 小节块的起音数 + RMS** —— 这就是段表的来源（也是 原结构配方（已删 2026-09-24） 的口径） | 见 §2 |
...
## 4b. "好听"的判据（**判据表在 `docs/THEME-PACK.md` §4b，共用一份**）

仿写的目标是"像参考曲的结构 **+** 好听"，而后一半是可量化的 —— 两组判据独立：
**段表（§2）管"像不像骨架"，碎音/时值管"听不听得下去"**。骨架再像，音符碎成一地照样是"杂乱"。

判据与量法**不在这里抄第二份**（PITFALLS 188：每份清单都会漂移），统一看
`docs/THEME-PACK.md` §4b，并用：

```bash
python scripts\note_dur_stats.py <曲.mid> [参考.mid] --floor 0.55
```

仿写场景的三条要点：旋律轨**时值中位 ≥0.25s**、**碎音率 <5%**、**打击轨 ~100% 是正常的（别拉长鼓）**。
⚠ 时值下限 = **延长不是合并**（起点不动）—— 与 `docs/RESTORE-METHOD.md` §2 那条不冲突（PITFALLS 206）。
