...
8. **还原/扒带任务：先跑 `similarity.py` 拿总分，再改**（用户 2026-09-15 要求"统一的判断标准"
   —— 之前逐个指标调参、按下葫芦浮起瓢，就是没有单一判据）：
   `similarity.py <参考音频> <我的音频> --bpm N --mine-json <song.json> --ref-chords <和弦.txt>`
   输出**总分 0–100 + 六项分项 + 差距最大的三项**：
   · **照抄即可**（和弦 30% / 节奏型格子 20% / 密度曲线 15%）—— **做不到是方法问题，别推给音源**；
     和弦维度**读 `song.json`、不要重扒**（扒带对 `m7↔7`、`sus4↔7` 只有 ~39% 分辨力，
     会把"照抄 100% 的曲子"评成 39 分）。
   · **其余**（倍频程 15% / 音色 10% / 段间变化 10%）—— 受音源与混音限制，别拿音量/EQ 硬追。
   ⚠ **但别一律归给音源**：实测某曲 160Hz–18kHz 曾低 **5–13dB**（80Hz 以下却 ±1dB），
...
9. **"听着乱/控制不住"先量三件事**（`docs/MAKE-IT-SOUND-ALIKE.md`）：同时发声的**乐器数**
   ...
   三处典型失控：鼓用全曲平均（不跟段落呼吸）· 扒谱把**泛音**当独立音（八度族 25~31%）·
