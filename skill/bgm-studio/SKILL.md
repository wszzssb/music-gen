...
## 2. 开工前先定这 8 件事（不定就会返工，每条都是实测踩出来的）

0. **首要标准：和谐、不突兀**（用户 2026-09-15 定）—— **优先于 CLAP happy 这类好听度指标，
   冲突时以和谐为准**。实测教训：为把 happy 0.44→0.55 换上颤音琴，用户立刻听出"有个乐器慢半拍"。
   ...
   **改音色/编配前先问"会不会让哪轨冒出来"。**
   **最好的对照是 `docs/CASE-BGM35.md`**（用户指定的最佳案例）。它可迁移的共性只有三条：
   ① **有明确的极静段**（它 −34.2dB，我们最静才 −18dB）② **收尾逐小节渐弱**
   （它 5 小节衰减到 −38dB，我们只有段末 1 拍）③ **调式色彩明确**（自然小调 b6；
   我们主题包多给 C/G/A major，色彩中性）。
   ⚠ **宽度/亮度/密度是曲风选择，别一概而论**：指定案例宽 0.473、高频 5/16；
   另一首 BGM36 宽 0.865、高频 14/16，**同样是好曲**。
   ⚠ **指标不许当差评依据**：指定案例 CLAP tense 0.76、BGM36 更是 **0.961**，
   而 AQA CE 都只有 7.1 —— 用户却认为它们是最好的一批。
