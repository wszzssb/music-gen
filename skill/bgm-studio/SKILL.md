...
0. **首要标准：和谐、不突兀**（用户 2026-09-15 定）—— **优先于 CLAP happy 这类好听度指标，
   ...
   **改音色/编配前先问"会不会让哪轨冒出来"。**
   **混音不是瓶颈** —— 实测我们 36 首 vs 权威素材 50 首的**分布**几乎重合：RMS 中位
   都是 **−16.90**、宽度 0.51 vs 0.48、质心 3101 vs 3463、三个频段差 ≤1.3dB。
   ⚠ **别拿单首下结论**：BGM35 宽 0.473、BGM36 宽 0.865，**0.39–0.66 整个区间都是商业水准**。
   **真正的差距是「同质化」**：权威的 P25–P75 跨度 RMS **4.8dB** / 质心 **1858Hz**，
   我们只有 **1.6dB / 914Hz** —— 用户说"这个文件夹里大部分都很好"，共性正是
   **每首都不一样、且都在合理区间内**。
   三条可迁移的共性（见 `docs/CASE-BGM35.md`）：① **有明确极静段**（它 −34.2dB，
   我们最静才 −18dB）② **收尾逐小节渐弱**（它 5 小节衰减到 −38dB，我们只有段末 1 拍）
   ③ **调式色彩明确**（自然小调 b6；我们主题包多给 C/G/A major，色彩中性）。
   ⚠ **指标不许当差评依据**：BGM36 CLAP tense **0.961**、AQA CE 只有 7.1，
   用户却认为这一批很好。
...
8. **还原/扒带任务：先跑 `similarity.py` 拿总分，再改**（用户 2026-09-15 要求"统一的判断标准"
   ...
   `python scripts\similarity.py <参考音频> <我的音频> --bpm N --mine-json <song.json> --ref-chords <扒好的和弦.txt>`
   输出**总分 0–100 + 六项分项 + 差距最大的三项**，维度已按"能不能做到一样"标注：
   ...
   补 800Hz 以上 +6dB 后平均差 **6.75 → 1.87dB**。
