...
**−1. 开工第一件事：跑 `audit.py` 拿"偏差项清单"**（用户 2026-09-16："能不能让最开始
   就发现所有要注意的元素"）—— 之前是**踩一个补一个**（乱→旋律静音→MIDI 力度），
   每条都由用户先听出来。四层一次跑完，逐项给"达标/偏差/**测不到**"三态；
   参考线与**8 条测不到、必须人耳**的项 → `docs/AUDIT-CHECKLIST.md`。
