...
ARR_KEYS = ('uku', 'piano', 'ep', 'strings', 'glock', 'bass', 'pad', 'arp',
            ...
            # GM 音源在 110+ 会明显"炸"。
            'perc_vel_max',
            # `glock_dur`（2026-09-16）：钟琴音长（拍），默认 0.5。
...
def build_events(d):
    ...
    for sec_i, sec in enumerate(d['sections']):
        ...
        for i in range(nbars):
            ...
            # ⚠ 时值必须**短**（2026-09-16，用户："听 midi 感觉声音怪怪的"）：
            #   原来写 `B - 0.1`（= 3.9 拍）——**钟琴是点状音色，长鸣 3.9 拍在高频区
            #   非常刺耳**，实测我方 Glock 时值中位 **3.900 拍**。
    ...
    # 让"呼吸口"真的空下来。
    # ⚠ **按轨区分**（2026-09-15 再修）：`variation` 与 `density` 是一对**互相拉扯**的指标 ——
    #   疏段压得越空，密度对比越大，但**段间质心波动**也越大（实测我的质心块间波动
    #   **853Hz** vs 原曲 **265Hz**，`variation` 因此卡在 26.7）。
    #   原曲的解法是：**疏的是"节奏/低音"，高频层一直在**（疏而亮）。
    #   所以**只让"密度类"轨（Bass/Piano/Hook）按 density 抽样**，
    #   **和声/高频层（Arp）全保留** —— 呼吸口"少而亮"，而不是"又少又暗"。
    # ⚠ 试过"只让密度类轨（Bass/Piano/Hook）抽样、和声层（Arp）全保留"——
    #   动机是让呼吸口"疏而亮"、稳住段间质心（原曲质心块间波动 265Hz，我 853Hz）。
    #   **实测反而更差**：`octave` 78.0 → 71.3、总分 71.9 → **70.7**。
    #   **整段删掉**，维持"所有轨都按 density 抽样"。
    #   （⚠ 踩过的坑：改成 `_density_tracks = ()` 想关掉它，但 `_tr not in ()` **恒为 True**，
    #    结果所有轨都走了"全保留"分支、抽样彻底失效，分数掉到 70.2。逻辑开关要用真条件。）
    # **还原模式**（`patterns.notes_extra_full`）：**不抽样**，逐音照写。
    # ⚠ 2026-09-26 **默认翻转为 True**（原来是"不写就抽样"）。
    #   抽样（把抄来的音符按 `arr.density` 逐小节砍上限：{1:4, 2:10, 3:18}）当年是为
    #   "让转录服从段落结构"设计的；但**扒带要的是忠实**，实测它砍掉的正是用户最在意的地方：
    #   `dear_good_friends` 抽样后全曲转录只留 54%（Piano 984→517 · Hook 482→265 · Bass 47→38），
    #   而用户点名"不一样"的 0–27s / 63–71s / 126s+ 全是 `density=1`（上限 4 音/小节）
    #   的段落，保留率仅 **13%~35%**；他没投诉的 `density=3` 段保留 50%~100%。
    #   翻成默认全量后用户原话："**V1_全量转录.mid 非常好**"。
    #   想回到抽样 → 显式写 `"notes_extra_full": false`（别再靠"不写"表达意图，
    #   `t_restore_notes_full` 会拦"扒带曲不声明"的情况）。
    #   注：`notes_extra` 为空时这个开关**完全无副作用** —— 生成类曲目不受影响。
    _full = (d.get('patterns') or {}).get('notes_extra_full')
    _full = True if _full is None else bool(_full)
