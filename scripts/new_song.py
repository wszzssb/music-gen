...
def build_from_theme(pack, short, seed=7, ncand=4, energy_gain=None):
    ...
    # **中段"呼吸口"极静段**（2026-09-18 补）：原案例文档（已删 2026-09-24） 实测它的
...
def melody_prog_pool(mel_tpl):
    ...
    根因正是池首的 **GM 0 钢琴衰减太快**（技能第 17 条实测：GM 0/1 从峰值掉 12dB 只要
    **0.19~0.25s**），而旋律写的是长音（1.35 拍）→ 听感"**写的是长音、只响 0.几秒**"。
    **GM 4 在同族内、起音实测 1ms、不掉 12dB**，是技能里点名的"持续型"解；
    它同样属**钢琴族保守音色**，所以"引子/主歌拿保守音色"这条初衷不变。
