...
LIMITS = {
    ...
    #   指针另有 catalog 显示预算（≈500 字符，**砍尾**），顺序已改为
    #   核心词 → 铁律 → 长尾词表（守卫 `docs_budget_and_skill_intact` 在守）。
    #   **压缩目标：回到 3950**（并入上面既有欠账；首选压 §3/§4 重复条目）。
    # 压缩进展（2026-09-18 同日）：**4256 → 4139（-117 tok）**。已砍的是纯指针条目
    #   （§3.11 整条只是指向 §2.9）+ §2.0/§2.8/§2.9b/§2.9c/§2.9d 的叙述性文字，
    #   **数字、工具名、坑编号、守卫名、用户原话一律保留**。
    #   **剩余 189 tok 需「下沉」而非硬砍**：把 §2 里的案例数字（BGM29 的 48.5%→69.9%、
    #   BGM35 的质心 1035/带差 14.11dB 等）挪进 `docs/CASE-BGM35*.md`，
    #   SKILL.md 只留结论 + 指针 —— 那本来就是它该有的分工（见 docs/CONVENTION.md §2）。
    'SKILL.md（音乐任务加载）': 4300,
