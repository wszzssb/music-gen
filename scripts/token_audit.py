...
LIMITS = {
    ...
    #   （鼓不跟段落呼吸 / 泛音当独立音 / melody 写成全局小节号→旋律轨静音）。
    #   同时把 §4 的工具清单并进 README 指针（重复清单已删）抵消了一部分。
    'SKILL.md（音乐任务加载）': 2800,
    ...
    # +100（2026-09-16）：登记 `similarity.py`（还原度总分，唯一判据）与 `band_match.py`
    #   （倍频程平衡对齐）；两者合占一行，`band_match` 的用法边界另见
    #   `docs/MAKE-IT-SOUND-ALIKE.md`（不占 README 篇幅）。
    'README.md（工具链总索引）': 6800,
