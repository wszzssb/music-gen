...
阈值（`--strict` 时非零退出，供 CI/自检用）：
  最长同音串 ≤4 · **整曲同音率 ≤45%** · 密度 ≥1.2 · 碎音 ≤8% · 强拍贴合 =100%（有强拍样本时）
⚠ **碎音这一维要看画像**：快曲（150BPM+）里十六分音符是正常语言，慢曲里才是截断伪影。
⚠ **同音率与最长串要一起看**：批量改音高造成的压平会被段落切碎（串长只有 2~3），
  只判串长会漏 —— 实测 16 个音写死同一音高时串长 2~3、同音率 100%。
"""
import glob
import json
import os
...
# 阈值有分布证据（`tools/same_rate_probe.py` 量全库 29 首）：中位 11.9% / P90 17.2% / 最大 18.5%
# → 取 45% 是"正常曲目的 2.5 倍"，能抓真压平、不误伤任何现有曲目。
MAX_SAME = 45.0
...
def issues(r, strict=False):
    ...
    # **整曲同音率**（`maxrun` 的补盲）：压平被段落切碎时串长很短，只有这一维看得见。
    if r.get('same', 0) > MAX_SAME:
        out.append('同音率%.0f%%' % r['same'])
    if r['dens'] < MIN_DENS:
