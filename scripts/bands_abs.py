...
def main():
    ...
    if bad:
        print('  提示：绝对差 >6dB 的频带 %s —— **先改音色/音量，不要硬推 EQ**（EQ 上限很低）'
              % ', '.join(bad))
        # ⚠ **列竞争假设，别跳步**（2026-09-20 实测教训 · RESTORE-METHOD §10 第 3 条）：
        #   当天就是从这个提示出发，**直接跳到**"模型 f_min=50Hz 的物理边界"，而真主因是
        #   "低音落在别的轨、而且是钢琴音色在弹"。所以把该逐条排除的候选**印在这里**
        #   （PITFALLS 209 的口径：判据要出现在它该起作用的那一刻）。
        print('  候选原因（**逐条排除，别直接认其中一个**）：')
        print('    · 该声部**根本没转录出来** → 按**音区**查、不要只按轨名：'
              '`transcribe_audit.py <分轨.wav> <我的.mid> --tracks "<真轨名>"`')
        print('    · **音色不对**（GM 里没有原曲那一层，如 sub 合成器 / 电子贝斯）'
              '→ 按音区拆轨换 program 后再量')
        print('    · **模型输入边界**：YMT3 的 `f_min=50Hz / f_max=8000Hz` → 20–40Hz 与 8kHz+ '
              '**物理丢失**（垫层/换音色最多补 1~2dB，别硬补）')
        print('    · 以上都排除后，才是**电平/编配缺层** → 改音色或音量，或用 '
              '`band_match.py --segments` 分段逐带对齐')
