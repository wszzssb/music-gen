# 14_retro_quest（主题模板包：retro / 复古游戏）

| 项目 | 值 |
|---|---|
| 模板依据 | **10 首同主题模板聚合**（`refs/themes/retro.json`） |
| 主题→风格 | chiptune/game16/game32（引擎预设 dance） |
| 速度·调式 | 140 BPM · G minor（模板中位） |
| 和声 | Gm6 D7 Gm6 D7（来源 window2） |
| 曲式 | 10 段 × 8 小节 = 72 小节 |
...
| `game16/7thsaga_bt2.mid` | game16 | 130 | https://www.vgmusic.com/music/console/nintendo/snes/7thsaga_bt2.mid |
...
- 音域 [67, 93] · 2.66 音/小节 · 级进 68% · 正拍 57%

## 频谱对齐画像（**不是模板**，只用于混音对标）

- **retro_mix**：6 份真实录音画像的**逐维度中位数**（多方参考，见坑 132）
  —— 响度 -16.9 dBFS · 宽度 0.660 · 质心 3250Hz

| 成员参考 | 评分 | 质心 | 来源 |
|---|---|---|---|
| `BGM15c` | 0.624 | 3338 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM15c.ogg） |
| `BGM16c` | 0.611 | 3250 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM16c.ogg） |
| `BGM16c_v2` | 0.611 | 3250 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM16c.ogg） |
| `bgm16c_new` | 0.611 | 3250 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM16c.ogg） |
| `BGM18` | 0.603 | 2043 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM18.ogg） |
| `BGM18b` | 0.603 | 1971 | 项目素材包 Bgm（同批提取的 galgame 商业 BGM）（BGM18b.ogg） |
...
& $py scripts\new_song.py 14_retro_quest --theme retro --seed 7
& $py scripts\make_song.py 14_retro_quest --check
```
