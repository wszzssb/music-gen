# 35_seaside_probe（主题模板包：seaside / 海边）

| 项目 | 值 |
|---|---|
| 模板依据 | **10 首同主题模板聚合**（`refs/themes/seaside.json`） |
| 主题→风格 | newage/folk/pop（引擎预设 acoustic） |
| 速度·调式 | 130 BPM · C major（模板中位） |
| 和声 | Em7 C6 F6 Cmaj7（来源 window4） |
| 曲式 | 8 段 × 8 小节 = 64 小节 |
...
- bgm01c：响度 -14.3 dBFS · 宽度 0.316 · 质心 3284Hz
...
& $py scripts\new_song.py 35_seaside_probe --theme seaside --seed 7
& $py scripts\make_song.py 35_seaside_probe --check
```
