...
> ## ⚠ 别整篇读这个文件 —— 按下面这张地图读你需要的 1~2 节（**节级行号见 `docs/DOC-MAP.md`**）
>
> | 你要做的事 | 读哪节 |
> |---|---|
...
| `probe_instruments.py` | **扒带第 ⓿ 步：逐段乐器编制表**（用户 2026-09-25 要求固化；**先出表、再写谱**）：四层证据 —— ① YMT3 乐器通道 ② 本地 Qwen2-Audio（`--qwen`，实测**会漏**）③a **各分轨的转录音数**（来源不丢）③b **分轨音频能量**（逐段 RMS 相对段内最响分轨，门限 −22dB 由分布**双峰**标定）③c **物理判据**（起音 10→90% · 起音后 300ms 衰减 · 颤音 3–9Hz **带内** RMS）。**能量是物理事实、标签与音数是模型产物**：YMT3 会把弦乐标成 `Acoustic Piano`，而那条钢琴轨实测比主奏**低 13dB** → 表里直接标 `⚠标签冲突(YMT3=…,该轨比主奏低 NdB)`。**已验证**：用户真值 8/8（人声吟唱段→`vocals` · 弦乐段→`other`）· 钢琴轨起音 **11–17ms** vs 弦乐/人声轨 **73–194ms** · 颤音负控（钢琴轨）**2.1¢** vs 正控 **5.2–6.8¢**。**未验证**：`other` 里弦乐 vs 合成器 pad、`vocals` 里人声 vs 主奏合成器（无标注样本，**不许当结论**）。颤音尺子 `--selftest` 是**硬门**（合成已知信号，不过就不许量真音频；当天它连拦下三次坏法）|
| `probe_bpm_layers.py` | **BPM 层级判定**（扒谱/还原**定速度必跑**）：三把独立尺子打同一份音频 —— ① 自相关候选层 + 每层支持度 ② 起音间隔(IOI)众数/中位 ③ 网格贴合（中位偏差 + <25ms 占比）；`--stems` 用 `drums` 分轨复算 · `--deep`（`.venv-ml`）加 percussive + DP beat tracking（带 IOI 的 CV）。**速度是层级不是单值**：半/双速/附点层天然都成立，选错会让**小节数翻倍、段落切分与逐小节鼓型全错位**（时间轴仍对）。实测 `siren_end` 三层都成立（193.6/96.8/143.55）→ 再用"每拍鼓音数 + 段长分布"拍板 |
| `restore_gap_fill.py` | **分段补漏/删音**（还原曲"有的地方不像/有音乐的地方没声音"时用）：拿 Demucs 分轨各自转录出的 MIDI，**逐小节**判定 —— 我们 0 音或 < 分轨 `--ratio` 的小节**补**，原曲静音（`--audio` + `--min-rms`）的小节**不补**且 `--prune-quiet` **删掉我们却有的音**。⚠ **别整曲一刀切**（用户 2026-09-25 定：分段落处理）；输出**逐段×逐小节改动表**。实测 `siren_end`：11 个完全没响的小节补回 177 音、结尾 2 个静音小节正确跳过（不补进去就是"该没声音的地方有声音"）|
...
| `loop_export.py` | **导出可无缝循环的素材**（这是音频大模型**结构上给不了**的：它没有小节/循环点概念）：按段落或小节取区间 → **尾音回绕**（把循环点之后的衰减尾叠回开头 —— 否则每循环一次就丢掉一截，实测 `20_piano_rain` A 段循环点之后 2.5 秒内的尾音只比正片低 **0.3dB**）→ 出 `loop/*.wav` + `loop.json`。`--no-wrap` 出对照版 · `--repeat 3` 出循环 3 遍的**试听版**（接缝暴露 2 次，**最终判据是耳朵**）· **`--stems` 逐轨导同一区间的循环素材 + 出参数绑定清单**（`base` 常驻底 / `accent` 可被参数驱动，接 Wwise/kibaudio 那类中间件） |
| `octave_audit.py` | **八度错误率审计** —— 还原/扒带的**第一道工序**（先量识别准不准）：八度错最会骗过频谱指标（BGM29 那轮"带差做到 0.83dB、看起来更好了"，而那批音 **25.3% 弹的是错八度**）。读法：看**"占全部音符"**那个数，只看"占未匹配"会被小分母放大 |
...
| `seg_contrast.py` | **逐段对比**：按 `song.json` 的段落计划逐段量「响度 / 频谱质心 / 5–18k 相对能量」出段表 —— 看段间起伏与"哪段亮"（`notes.md` 里那张段间表就是它出的） |
| `probe_sustain.py` | **"响多久 / 谁跳出来 / 流畅不流畅 / 有没有突然冒出来的声音"**四合一：① 独奏窗口的"写的比响的长"（只信独奏窗口 —— 多轨齐奏时音头会被别的轨借走，读数全错）② "镫"候选榜（纯 MIDI = `harmony_check.stab_candidates`）③ **流畅度**（音间空隙 / 断奏；用户"**重要是更流畅**"）④ **突兀声**（孤立脉冲中**没有 MIDI 起音对应**的；⚠ 对齐窗口 ≥120ms —— GM 起音延迟实测 42~78ms，给窄了会 5/5 误报）→ 坑 235/236 |
...
| `doc_map.py` | **文档地图**（大目录=主题域 → 小目录=文档节**+行号**+体量）：`--stdout` 打印 · `--check` 校验是否过期 · `--list` 盘点所有文档。产出 `docs/DOC-MAP.md`（**生成物，别手改**）——长文档（`HISTORY` 38k / `PITFALLS` 30k）一次读不完时先查它 |
| `mutation_check.py` | **变异测试**：注入故障验证自检**真会报警**（改过检查项就跑它；新检查必须配注入用例） |
| `pitfalls_archive.py` | **PITFALLS 归档**：把较早条目剪切进 `PITFALLS-ARCHIVE.md`（**编号不变**）。⚠ 它开头就记着自己第一版**不幂等**、被重复跑坏过一次的现场 —— 跑之前先读它 |
...
| `cleanup_transcribe.py` | **转录产物清理**（`transcribe_to_song` 之前；残片/抖动）：三档判据全部带音频证据 —— A 残片（<60ms 且被同音高「≥3 倍长、覆盖 ≥80%」的音覆盖）删 · B 抖动（起点差 <50ms）按 **RMS 起音跃升** 判，<2.0 才并 · C 双长音重叠（都 ≥60ms）**一律不动**。判据比 `mason369/music-to-midi` 硬：它 `_smooth_vibrato` 无条件统一半音（该材料 61 处里 **39 处两音都真实在响**）、`_merge_close_notes` 无条件并 gap≤10ms（137 处里 **113 处是真实踏板重弹**）。`--selftest` 是硬门（合成信号，不过不许动文件） |
...
| `unison_guard.py` | **补音 / 加层之前的"同刻打架"守卫**（同刻 + 音程 Δ0 + 两边力度 ≥90）。**为什么**：`siren_end2` 的 v19 把 47 个弦乐越界低音搬进**已经在弹的** Bass 轨，指标"20–40Hz +1.5dB"看着变好、用户听感却是"**太吵太乱**"——48 个音里 25 个与既有贝斯**同度**、13 个大二度，力度 96–106 顶格（回退后用户确认"好多了"）。⚠ **它治的病和"复音上限"不是同一个**：那些音塞进去后 Bass 轨峰值**仍是 4**（= music-to-midi 的贝斯上限），上限机制看不见它。用法：`--base <基准.mid>` 做**补音体检**（真病例 Bass 轨命中 **89% vs 该轨基线 19%**，4.7 倍）· `--apply` 只对 Δ0 去重（**Δ2 一律不动**，那可能是真实二度和声）· `--selftest` 是硬门。⚠ 判据当天被自检抓出**三类假读数**：跨格重复计数（1 处报成 8）· 按 (轨名,音高) 去重（86% 压成 10%）· 基线没排除音自身（虚顶 96%）；**鼓轨必须排除**（GM 鼓的"音高"是乐器编号） |
| `bpm_fit.py` | **速度不是一个标量**：拍点证据 → 拟合 → **双闸门** → 半/双拍归一化（对标 music-to-midi 的 `telknet_beat_grid_v12`）。`metrics.detect_bpm` 快、只给候选；这个慢（要 beat tracking）、给**判据**：原点约束最小二乘 + minimax 两种拟合、**最大偏差既不能超 3/8 拍也不能超 40ms**、可疑时**降级并输出带数字的警告**而不是硬给一个数。半/双拍家族按 ¼ ⅓ ½ **⅔** ×1 **×1.5** ×2 ×3 ×4 逐层过闸门，都过就报"有歧义、必须显式选一层"。**实测**（自检正控）：标准脉冲串 ±0.3 BPM · **连奏型测出 106.19**（那正是我们踩过"106→154.3"的模式）· 对照曲已知 132 测出 **131.99**；负控：白噪声不过闸门（94ms）、拍点不足直接报"判不了"。⚠ `siren_end2` 原曲真值 145.96 在音频层拟合出 98.2（比值 **1.486 = ×1.5 家族**）且闸门不过 → 工具明说"别拿这个数当速度"（技能 §3-2"速度一律取自 MIDI"仍然成立） |
| `timbre_audit.py` | **逐段"音色对不对"的自动体检**（把 `probe_instruments` 的编制表接到我方编配上）。`chroma` 只能证明"音高内容大体对"（0.85~0.95，区分度低），而用户听到的"不像"**全在音色层**、一直没有尺子 → 每改一版都要等人耳。判据：① **高频嘶声**＝我方该段 2–6kHz 占比 > max(3%, 原曲×3)；② **形态不符**＝原曲软起音（>150ms，垫子/弦乐）而我方硬起音（<30ms，击弦）；③ 质心差 3 倍**仅供参考**（原曲是分轨质心、我方是混音质心，本不可比 —— 按它判会把认可版也报成 8/25 段）。**实测独立复现了用户的判断**：v22a（认可）**0 段**报嘶声、v25（"蚊子叫"那版）**8 段**，峰值 **28.58% vs 原曲 1.9%**。`--selftest` 是硬门 |
| `filter_by_stem.py` | **按分轨能量筛掉"这条轨根本没在响"的段落里的音**（对所有轨）。**为什么**：`siren_end2` 开头 18 秒弹了 **42 个凭空贝斯音**，而原曲那两段 `h6_bass` 是 **−87.4/−80.8dB**（差 50~60dB，等于没有）—— 合并器从不检查"这条分轨在这一段有没有声音"，转录在近乎静音的素材上照样出音。⚠ 门限**由分布双峰标定、空档 <12dB 就拒筛**（实测贝斯两簇空档 **31.8dB**、其它轨只有 3.7~11.0dB）—— 硬挑门限就是拍数字。`--apply` 逐文件留 `.pre_stemfilter.bak` |
...
| `stem_export.py` | **按轨（stem）导出 MIDI**：每条轨一个 `NN-<轨名>-<BPM>BPM.mid` + `stems.json` 清单 + `--zip`（临时文件 + `os.replace` 原子发布 + **testzip 回读**）。**每条轨都带 tempo/拍号** —— 单轨 MIDI 少了 tempo，DAW 里会**回落 120BPM**（小节线全错）。每导一条跑**四重自证**（照 music-to-midi 的 `midi_editor` 口径）：① tempo/拍号 ② 音符指纹 ③ 透传事件（CC/program/marker）④ **幂等：导出文件再导出一遍逐字节相同**。实测 `siren_end2` 6 轨全 PASS。⚠ 它的前提是 `midi_file.export_midi` 的 `with_meta_head=(i == 0)` 看**列表位置**而非 `index` 字段（单元素列表必写 tempo）—— 这条性质由 `selftest.t_stem_export_tempo_carry` 钉着 |
