...
> | 你要做的事 | 读哪节 |
> |---|---|
> | **写一首新歌（唯一合规路径）** | **`docs/THEME-PACK.md`**（主题模板包 = 依据；混音目标）+ 下面 §1 的四步 |
...
| `groove_probe.py` | **微时序/swing 探针**：谱通量起音 → 偏离最近 16 分格的量（ms）→ 逐格偏移表（可直接套 MIDI）。实测 BGM35 是贴格子的直拍（−4.9ms、无 swing） |
