...
├── scripts\             46 个脚本：引擎 + 分析/体检/渲染 + 自检/预演/变异/成本审计
...
| `rehearsal.py` | **新歌预演**：全新参考曲 × 5 套风格 + 边界情况端到端（含"调参确实改善"等断言） |
...
| `melody_gen.py` | **按画像生成旋律**：时值/落点/级进率/音域都从画像抽样，强拍强制吸附和弦音（生成后打印与画像的对照） |
...
- **自检全绿**；**注入故障全部被抓**（防线不是摆设）：
  ...
  （自检项数随每轮修 bug 增长：现在是 **53 项**，含建议口径、速度来源、编码安全等检查。）
...
3. **拍号**：引擎到处假设 4/4（`bar = 4 * beat`），3/4、6/8 未支持。
4. **极端规模**：超过 ~3 分钟的长曲、<60 或 >200 BPM 未测。
5. **平台**：只在 Windows 验证（`play_midi` 走 winmm、fluidsynth 是 win64 二进制）。
6. **老曲目（01–04）的编配**只经过"和弦名 vs 音集"校验，没有音乐性校验。
...
8. **音频测速（`detect_bpm`）仍不可靠**：本工具链已改成"速度取自 MIDI/`--bpm`"，但
   `profile_ref.py` 扒新参考曲时仍靠它；扒出来的速度要**人工核对**（与 `probe_style` 的
   逐拍节奏型对不上就是判错了）。倍频关系（75↔150）可以用 `--bpm` 纠正。
9. **你的播放器/DAW**：OGG/MIDI 在你用的软件里是否正常，只有你能确认。
10. **预演的"对齐 dB"只能当参考**：小样只有 4 小节、配器是通用骨架，而参考曲是完整制作，
    实测差距常在 5–20dB（慢速参考曲如 64BPM 的 BGM01/BGM10 更明显）。预演证明的是
    **管线通不通 + 调参有没有效果**，不是"写得像"。真正的对齐要看成品曲的成绩单。
...
```
songs\01_ac150_seaside\   summer_seaside.mid / .wav / .ogg / _sf.wav / _sf.ogg + notes.md
songs\02_dn128_drive\        drive_pop.mid / _sf.wav / _sf.ogg + notes.md
songs\03_ac76_garden\      garden_warm_test.* + notes.md
songs\04_dn92_garden_theme\ pure_garden_theme.* + notes.md
```

命名约定：`_sf` = 真音源版（FluidSynth + GeneralUser GS）；不带 `_sf` 的是自写合成器试听版。
**MIDI 是上限最高的交付**（可换任何更好的音源），OGG 是能直接听的成品，WAV 可重编码。

## 4. 踩过的坑（完整台账见 `PITFALLS.md`（现行 41–92）+ `PITFALLS-ARCHIVE.md`（1–40），按编号查）
