...
4. **极端规模**：**>3 分钟的长曲已交付**（`11_dn75_neon` 307s、`13`/`14` 281s、`12`/`15`/`17`/`22`
   237–256s，一律后台渲染）；**<60 / >200 BPM 仍未真正支持** —— 实测 55BPM 的曲子会被自动测速
   报成 111（坑 104），只能靠 `--bpm` 人工钉死。
...
8. **音频测速（`detect_bpm`）仍不可靠**：本工具链已改成"速度取自 MIDI/`--bpm`"，但
   ...
   逐拍节奏型对不上就是判错了）。倍频关系（75↔150）可以用 `--bpm` 纠正。**60–180 BPM 之外**
   的曲子原先被静默折进窗口（55→111），现在会另报 `info['window_alt']` 并要求 `--bpm` 核对；
   **>180 BPM 仍可能落在三拍关系上不报**（坑 104）。
