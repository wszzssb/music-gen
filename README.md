...
11. **旋律"同质化"（已量化，未治本）**：全库**跨曲共享旋律片段只有 0.5%**（不是照抄），
    但**旋律语言**高度重合 —— 落点分布 93–99%、时值分布 87–98%，因为 **10 首只用了 2 份旋律画像**
    （`BGM33` 画像养 11–15、`BGM16c` 画像养 16/23/25/27/30）。听感上的"每首都像"来自这个。
    守卫 `melody_distinct`（跨曲共享 ≤5%）；治本要改 `melody_gen`（每首用不同画像 +
    对库里已有旋律做去重筛选 + 画像补个性维度），成本高，见 HISTORY。
