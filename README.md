...
| `timbre_audit.py` | **逐段"音色对不对"的自动体检**（把 `probe_instruments` 的编制表接到我方编配上）。`chroma` 只能证明"音高内容大体对"（0.85~0.95，区分度低），而用户听到的"不像"**全在音色层**、一直没有尺子 → 每改一版都要等人耳。判据：① **高频嘶声**＝我方该段 2–6kHz 占比 > max(3%, 原曲×3)；② **形态不符**＝原曲软起音（>150ms，垫子/弦乐）而我方硬起音（<30ms，击弦）；③ 质心差 3 倍**仅供参考**（原曲是分轨质心、我方是混音质心，本不可比 —— 按它判会把认可版也报成 **10/25 段**）。**实测独立复现了用户的判断**：v22a（认可）**3 段**报嘶声（S10/S11/S16）· 形态 5 段、v25（"蚊子叫"那版）**8 段**，峰值 **28.58% vs 原曲 1.9%**。`--selftest` 是硬门。⚠ v22a 这个数先后写过"**0 段**"与"**4 段**（S13/S17/S21/S23）"—— **两个都没有可复现证据**（拿别的音频当 mine / 段号抄错）；2026-09-25 用**修好段界的尺子**重测 = **3 段（S10/S11/S16）**，且交付 `_sf.ogg` 与 `rend/v22a.ogg` **逐字节相同**、与 `rend/v22a.wav` 读数一致。**用前重测，别抄旧数** |
| `arrange_voices.py` | **按逐段编制表分配发声轨**（B 线第 2 条：把 `timbre_audit` 的"体检判据"接成"编配的输入"）。替代三个**全局**规则（`four_inst` 收成 4 件 · `kill_pad` 全曲关垫层 · `voice_split` 每刻最高音→小提琴）—— 它们从不读转录证据，把原曲那层合成器垫切给了弦乐。判据：`lead==other` 且起音 >150ms → 该段音**全归 Pad**；`lead==vocals` 且软起音 → 旋律段（默认不动内容）；其余 → 一个音不改；**硬不变量：Pad 的音只进不出**（依据：v22a 那 663 个 Pad 音 100% 是链④ Pad 的子集）+ 跨轨完全重复去重（优先级 Strings>Piano>Pad>Hook）。**实测**（同一份链④输入、**只换这一步**）：高频嘶声段 **8→4**、S01/S02 chroma **0.875/0.876 → 0.914/0.916**（v22a 0.933/0.929；残差已定位到链①的贝斯来源筛选）。⚠ 判据全部**段级**、逐段打印决策表（坑 20/255） |
| `restore_oneshot.py` | **扒带"一键到底"**（`song.json` 之后的那半段；`transcribe_ymt3.py` 已管前一半）。阶段：`probe`（编制表）→ `repair`（`arrange_voices`+`merge_sustain`+静音段体检）→ `vel`（力度写回）→ `arrange`（段级 CC7/力度）→ `render` → `audit`（三把尺子）。**验收实测**：只给"纯转录产物"，一条命令跑完 → **\|逐段 RMS 差\| 2.15 · chroma 0.9118 · 嘶声 2 段**（12 轮手工最佳 2.07 / 0.9072 / 1 段；老链同输入 1.72–1.86 / 0.9105 / **3~8 段**）。⚠ 默认**不删音、不开段级音色**（两条都是实测负结果） |
| `merge_sustain.py` | **把持续层被转录切成的"音头串"并回长音**（改 `song.json`）。两道门：间隔 ≤1 拍 **且** 原曲该音高在那个间隔里的能量 ≥ 两侧的 **30%**（原曲真断了的地方不并）。实测 `Strings` **1139→333 音 · 时值中位 169→728ms**（用户认可保留） |
